...
$sql="SELECT * FROM reservation WHERE YEAR(time) = '$reportYear' ";
...
<tbody>       
    ...
    $result = mysqli_query($con,"SELECT * FROM reservation WHERE YEAR(time) = '$reportYear' ");
    $count = mysqli_num_rows($result);

echo "<h3><center> Reservations From $reportYear</center></h3><br><br>";
...
while($row=mysqli_fetch_array($result)){
    ...
    <?php if($count = 0){ ?>
        <script type="text/javascript">
            alert("Unable To Generate Report For <?php $reportYear?>");
            history.back();
	</script>
    <?php }?>
    </tbody>
