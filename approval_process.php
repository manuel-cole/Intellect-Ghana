...
$trim = $_POST['trim'];
$cost_after_duty = $_POST['cost_after_duty'];
...
while($row=mysqli_fetch_array($result)) {
	...
	if (array_key_exists($reservation_id, $_POST["check"])) {
		...
		if ($ischecked==1) {
			...
			$sql = "INSERT INTO approved_reservation (approved_id, cost_after_duty, car_type, trim, year_made, exterior_colour, options, km, quantity, reserver)
					SELECT reservation_id, cost_after_duty, car_type, trim, year_made, exterior_colour, options, km, quantity, reserver
...
while($row=mysqli_fetch_array($result)) {
	...
	if (array_key_exists($reservation_id, $_POST["check"])) {
		...
		if ($ischecked==1) {
			...
			VALUES ( '$car_type', '$trim', '$cost_after_duty', '$year_made', '$exterior_colour', '$options', '$km', '$reserver' )";**/
			//$sql = "INSERT IGNORE INTO disapproved_reservation SELECT reservation_id, cost_after_duty, car_type, trim, year_made, exterior_colour, options, km, reserver FROM general_reservation";
			$sql = "INSERT INTO unapproved_reservation (unapproved_id, cost_after_duty, car_type, trim, year_made, exterior_colour, options, km, quantity, reserver)
					SELECT reservation_id, cost_after_duty, car_type, trim, year_made, exterior_colour, options, km, quantity, reserver
					FROM general_reservation 
					WHERE reservation_id = $reservation_id";
			$sql2 = "DELETE FROM general_reservation 
...
<link rel="stylesheet" href="css/lightbox.css">
 ...
 <div class="row">
        <div class="">     
            <h2> <center>Car Reservation Unapproved Successfully.</center></h2>
       <br>  
       <h4><center>Click <a href="regional_head_page.php">Here</a> to go back.</center></h4>
        </div>
        </div>
